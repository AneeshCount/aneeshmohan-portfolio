...
export const STR = {
  EN: {
    ...
    projects: [
      { tag: 'Flagship · Agentic AI', blurb: 'An EV charging and energy-management SaaS built from the ground up. Agentic watchdogs continuously monitor chargers, batteries and solar, while an LLM chat layer lets users control the system in plain language: start a vehicle charging, shift load to solar, or push a schedule, and the agents execute.', label: 'demo.ridergy.com' },
      ...
      { tag: 'RBAC Business Dashboard', blurb: 'A role-based business dashboard proving out a Laravel + jQuery stack end to end: session-based auth with three enforced roles, live AJAX KPI dashboards with filtering, charting and CSV export, and a swappable payment-gateway service, sandbox today, real Stripe with a one-line config change.', note: 'Demo logins (password: password): admin@demo.test · editor@demo.test · viewer@demo.test · free-tier hosting, first load after idle can take 20-30s', label: 'Live demo', howTo: [
  ...
  DE: {
    ...
    projects: [
      { tag: 'Flaggschiff · Agentische KI', blurb: 'Ein SaaS für EV-Laden und Energiemanagement, von Grund auf gebaut. Agentische Watchdogs überwachen Ladepunkte, Batterien und Solar, während eine LLM-Chat-Ebene das System in Alltagssprache steuert: Fahrzeug laden, Last auf Solar verschieben, Zeitplan setzen, die Agenten führen aus.', label: 'demo.ridergy.com' },
      ...
      { tag: 'RBAC-Business-Dashboard', blurb: 'Ein rollenbasiertes Business-Dashboard, das einen Laravel- + jQuery-Stack end-to-end demonstriert: sitzungsbasierte Anmeldung mit drei durchgesetzten Rollen, Live-AJAX-KPI-Dashboards mit Filterung, Diagrammen und CSV-Export, sowie ein austauschbarer Payment-Gateway-Service, heute Sandbox, mit einer einzigen Konfigurationsänderung echtes Stripe.', note: 'Demo-Logins (Passwort: password): admin@demo.test · editor@demo.test · viewer@demo.test · Free-Tier-Hosting, erster Aufruf nach Inaktivität kann 20-30s dauern', label: 'Live-Demo', howTo: [
        'Als admin@demo.test anmelden, um die vollständige RBAC-Erfahrung inklusive Benutzerverwaltung zu sehen.',
        'Das KPI-Dashboard nach Kategorie, Region und Zeitraum filtern, alles per AJAX, ohne Neuladen der Seite.',
        'Die gefilterten KPI-Daten direkt aus dem Dashboard als CSV herunterladen.',
        'Eine Testzahlung mit pm_card_visa (erfolgreich) oder pm_card_decline (abgelehnt) ausprobieren, verarbeitet von einem Sandbox-Gateway ohne externe Aufrufe.',
      ] },
  ...
  ES: {
    ...
    projects: [
      { tag: 'Buque insignia · IA agéntica', blurb: 'Un SaaS de carga de vehículos eléctricos y gestión energética construido desde cero. Watchdogs agénticos vigilan cargadores, baterías y solar, mientras una capa de chat LLM permite controlar el sistema en lenguaje natural: cargar un vehículo, mover carga a solar o fijar un horario, y los agentes ejecutan.', label: 'demo.ridergy.com' },
      ...
      { tag: 'Dashboard de negocio con RBAC', blurb: 'Un dashboard de negocio basado en roles que valida un stack Laravel + jQuery de punta a punta: autenticación por sesión con tres roles aplicados, dashboards de KPI en vivo por AJAX con filtros, gráficos y exportación a CSV, y un servicio de pasarela de pago intercambiable, sandbox hoy, Stripe real con un solo cambio de configuración.', note: 'Logins de demo (contraseña: password): admin@demo.test · editor@demo.test · viewer@demo.test · hosting de nivel gratuito, la primera carga tras inactividad puede tardar 20-30s', label: 'Demo en vivo', howTo: [
        'Inicia sesión como admin@demo.test para ver la experiencia completa con RBAC, incluida la gestión de usuarios.',
        'Filtra el dashboard de KPI por categoría, región y rango de fechas, todo por AJAX, sin recargar la página.',
        'Descarga los datos de KPI filtrados como CSV directamente desde el dashboard.',
        'Prueba un pago con pm_card_visa (exitoso) o pm_card_decline (rechazado), gestionado por una pasarela sandbox sin llamadas externas.',
      ] },
  ...
  FR: {
    ...
    projects: [
      { tag: 'Vaisseau amiral · IA agentique', blurb: "Un SaaS de recharge VE et de gestion d'énergie construit de zéro. Des watchdogs agentiques surveillent bornes, batteries et solaire, tandis qu'une couche de chat LLM pilote le système en langage courant : lancer une charge, basculer sur le solaire, fixer un planning, et les agents exécutent.", label: 'demo.ridergy.com' },
      ...
      { tag: 'Dashboard métier RBAC', blurb: "Un dashboard métier basé sur les rôles qui valide un stack Laravel + jQuery de bout en bout : authentification par session avec trois rôles appliqués, dashboards KPI en direct par AJAX avec filtres, graphiques et export CSV, et un service de passerelle de paiement interchangeable, sandbox aujourd'hui, vrai Stripe avec un seul changement de configuration.", note: 'Identifiants de démo (mot de passe : password) : admin@demo.test · editor@demo.test · viewer@demo.test · hébergement gratuit, le premier chargement après inactivité peut prendre 20-30s', label: 'Démo live', howTo: [
        "Connectez-vous en tant qu'admin@demo.test pour voir l'expérience RBAC complète, y compris la gestion des utilisateurs.",
        'Filtrez le dashboard KPI par catégorie, région et plage de dates, le tout en AJAX, sans rechargement de page.',
        'Téléchargez les données KPI filtrées en CSV directement depuis le dashboard.',
        'Testez un paiement avec pm_card_visa (réussi) ou pm_card_decline (refusé), traité par une passerelle sandbox sans appel externe.',
      ] },
